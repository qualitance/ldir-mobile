        ...
        link: function(scope, element, attrs, controller) {
            ...
            mapController.getMap().then(function(map) {
                ...
                getLayers().then(function(layers) {
                    ...
                    var oldMarkers = leafletScope.markers;

                    scope.$on('reloadMarkers', function () {

                        //check if new markers, else load all
                        if (Object.keys(leafletScope.newMarkers).length > 0) {
                            var newMarkers = {};
                            angular.forEach(Object.keys(leafletScope.newMarkers), function (key) {
                                //if marker already exists update it, else add a new one
                                if (leafletScope.markers.hasOwnProperty(key)) {
                                    var marker = _getLMarker(leafletMarkers, key, leafletScope.newMarkers[key].layerName);
                                    updateMarker(leafletScope.newMarkers[key], leafletScope.markers[key], marker, name, leafletScope, layers, map);
                                } else {
                                    newMarkers[key] = leafletScope.newMarkers[key]
                                }
                            })
                            //add all new markers if ther are any
                            if (Object.keys(newMarkers).length > 0) {
                                _addMarkers(newMarkers, leafletScope.markers, map, layers, leafletMarkers, leafletScope,
                                    watchOptions, undefined, leafletScope.markers);
                                leafletScope.markers = angular.extend(leafletScope.markers, newMarkers)
                            }
                        } else {
                            _create(leafletScope.markers, oldMarkers);
                            oldMarkers = leafletScope.markers;
                        }
                    });
                });
            });
